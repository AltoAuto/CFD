...
#### Student Implementation Guidance (student branch)
The student branch leaves core k–omega routines as stubs. Use these as guidance
for correct integration with the solver (names/shapes/returns only; choose constants
yourself):
- `_k_omega_defaults()`  
  Guidance: define the parameter names used by the model and return them.  
  Returns: `params` dict with keys: `alpha`, `beta`, `beta_star`, `sigma_k`,
  `sigma_omega`, `k_min`, `omega_min`, `nu_t_coeff`, `omega_wall_coeff`.
- `eddy_viscosity(fields, cfg)`  
  Inputs: `fields["k"]`, `fields["omega"]`, `fields["nu_t"]` shaped
  `(count_i + 2, count_j + 2)`.  
  Guidance: update `fields["nu_t"]` in place using `k`/`omega` with minima.  
  Returns: `fields["nu_t"]` (same ghosted shape).
- `sources(fields, grad_u, grad_v)`  
  Inputs: `grad_u`, `grad_v` shaped `(count_i, count_j, 2)` (interior only).  
  Guidance: compute production/dissipation using `fields["nu_t"][1:-1,1:-1]` and
  safe minima for `k`/`omega`.  
  Returns: `(source_k, source_omega)` shaped `(count_i, count_j)` (interior only).
- `effective_diffusivity(nu, nu_t, field="k")`  
  Inputs: scalar `nu`, `nu_t` shaped `(count_i + 2, count_j + 2)`.  
  Guidance: return effective diffusivity for `field` without mutating inputs.  
  Returns: array compatible with slicing `[1:-1, 1:-1]`.
- `_apply_komega_wall(fields, mesh, boundary, index_range, nu, params)`  
  Guidance: use `bc._boundary_geometry(...)` to compute wall distance, apply
  boundary values via `bc.set_dirichlet`, and (if needed) overwrite the adjacent
  interior cells using `bc._boundary_indices(...)`.  
  Returns: `None` (mutates fields in place).
